...
Go APIs are available. See [the Go API document](/api) for more details.
...
| `actionlint-system` | Uses system-installed `actionlint` command. The command is necessary to be [installed manually](/install). |
...
[Checks](/checks) | [Installation](/install) | [Configuration](/config) | [Go API](/api) | [References](/reference)
